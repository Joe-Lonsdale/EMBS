...
public class Processor extends TypedAtomicActor{
	...
	protected TypedIOPort trigger;
	protected TypedIOPort queue_len;
	...
	public Processor(CompositeEntity container, String name) throws NameDuplicationException, IllegalActionException {
		...
		queue_len = new TypedIOPort(this, "queue_len", false, true);
		busy = false;
	...
	public void fire() throws IllegalActionException{
		if(trigger.hasToken(0)) {
			Token x = trigger.get(0); // Check if the actor was fired to prompt for a queue length.
			queue_len.send(0, new IntToken(queue.size()));
		} 
		else {
			Token u, q = new Token();
